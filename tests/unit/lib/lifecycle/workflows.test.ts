...
const { mockExecAsync, mockClearReviewStatus, mockResetPostMergeState, mockSetCanonicalState } = vi.hoisted(() => ({
  mockExecAsync: vi.fn().mockResolvedValue({ stdout: '', stderr: '' }),
  mockClearReviewStatus: vi.fn(),
  mockResetPostMergeState: vi.fn(),
  mockSetCanonicalState: vi.fn(),
}));
...
vi.mock('../../../../src/lib/lifecycle/reconciler/index.js', () => ({
  setCanonicalState: mockSetCanonicalState,
}));
...
describe('workflows', () => {
  ...
  describe('closeOut', () => {
    ...
    it('should delete the workspace, complete vBRIEF, close GitHub, and swap verifying labels during configured close-out', async () => {
      ...
      expect(calls.some(([command, args]) => command === 'gh' && Array.isArray(args) && args[0] === 'issue' && args[1] === 'close' && args[2] === '100')).toBe(true);
      expect(calls.some(([command, args]) => command === 'gh' && Array.isArray(args) && args[0] === 'issue' && args[1] === 'edit' && args.includes('--add-label') && args.includes('closed-out'))).toBe(true);
      expect(mockSetCanonicalState).toHaveBeenCalledWith('PAN-100', 'merged');
    });
    ...
    it('should apply closed-out and delegate workflow label removal to the reconciler', async () => {
      const ctx = {
        issueId: 'PAN-100',
        projectPath: testDir,
        github: { owner: 'eltmon', repo: 'panopticon-cli', number: 100 },
