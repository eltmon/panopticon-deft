/**
 * Turn diff tree — builds a collapsible directory tree from a flat file list.
 * Mirrors T3Code's apps/web/src/lib/turnDiffTree.ts for 1:1 upstream compatibility.
 */

import type { TurnDiffFileChange } from '../components/chat/chat-types'
...
function toTreeNodes(directory: MutableDirectoryNode): TurnDiffTreeNode[] {
  const subdirectories: TurnDiffTreeDirectoryNode[] = [...Array.from(directory.directories.values())]
    ...
    .map((subdirectory: TurnDiffTreeDirectoryNode) => compactDirectoryNode(subdirectory))
