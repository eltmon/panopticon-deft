...
function findGhIssueCreate(tokens) {
	for (let index = 0; index <= tokens.length - 3; index += 1) if (tokens[index].text === "gh" && tokens[index + 1].text === "issue" && tokens[index + 2].text === "create") return index;
	return -1;
}
function containsFlywheelRunId(body) {
...
function buildTrailer(env) {
	...
	return `\n\n${[
		...
		discoveredIn ? `Flywheel-Discovered-In: ${discoveredIn}` : void 0
	].filter((line) => line !== void 0).join("\n")}\n`;
}
function replaceSpan(command, start, end, replacement) {
	return `${command.slice(0, start)}${replacement}${command.slice(end)}`;
}
function findFlag(tokens, ghIndex, flagName) {
	for (let index = ghIndex + 3; index < tokens.length; index += 1) {
		const token = tokens[index];
		if (token.text === flagName) return {
			flag: token,
			value: tokens[index + 1],
			inline: false
		};
		if (token.text.startsWith(`${flagName}=`)) return {
			flag: token,
			inline: true
		};
	}
...
function inlineFlagValue(flag) {
	const equalsOffset = flag.text.indexOf("=");
	if (equalsOffset === -1) return null;
	const valueStart = flag.start + equalsOffset + 1;
	return {
		value: flag.text.slice(equalsOffset + 1),
		valueStart,
		valueEnd: flag.end
	};
}
function appendBodyFlag(command, tokens, ghIndex, trailer) {
...
function writeTempBodyFile(body) {
	const path = join(mkdtempSync(join(tmpdir(), "pan-gh-issue-body-")), "body.md");
	writeFileSync(path, body, "utf-8");
	return path;
}
function appendBodyFileFlag(command, tokens, ghIndex, trailer) {
...
function findPipeBefore(command, beforeIndex) {
	let quote = null;
	let escape = false;
	let pipeIndex = -1;
...
function appendHereDoc(command, trailer) {
	const match = command.match(/(<<-?\s*['"]?([A-Za-z_][A-Za-z0-9_-]*)['"]?[^\n]*\n)([\s\S]*?)(\n\2\s*)$/);
	if (!match || match.index === void 0) return null;
	const body = match[3];
	if (containsFlywheelRunId(body)) return command;
	const bodyStart = match.index + match[1].length;
	return replaceSpan(command, bodyStart, bodyStart + body.length, `${body}${trailer}`);
}
function appendStdinBody(command, ghStart, trailer) {
...
function buildUpdatedInput(command) {
...
function rewriteGhIssueCreateCommand(command, env = process.env) {
...
function runGhIssueTrailerHook(input, env = process.env) {
	let event;
	try {
	...
	const command = event.tool_name === "Bash" ? event.tool_input?.command : void 0;
...
//# sourceMappingURL=gh-issue-trailer-hook.js.map